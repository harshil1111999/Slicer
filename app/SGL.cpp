...
bool find_slices(int slicing_line_number, string start, string end) {
    ...
    for(int i=low;i<=high;i++) {
        ...
        if(!flag || do_clone || (flag && j < line.length() && (line[j] == '{' || line[j] == '}'))) {
            ...
            function_definition.find(function_call[number]) != function_definition.end()) {
                find_slice(number);
...
void find_trace() {
    ...
    while(!fin.eof()) {
        ...
        if(check == 1) {
            ...
            string temp = find_function_name(line);
            if(function_definition.find(temp) != function_definition.end()) {
