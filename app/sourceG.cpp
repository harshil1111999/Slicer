...
void add_lines_from_header_file(string temp) {
    ...
    while(!fin.eof()) {
        ...
        if(temp == "include") {
            ...
            if(fin2.is_open()) {
                fin2.close();
                add_lines_from_header_file(temp3);
            } else {
                output<<line + "\n";
            }
        } else {
...
int main() {
    ...
    for (auto it : code)
    {
        slice << it.second->line_number << ":" << it.second->statement << "\n";
        CD << it.second->line_number << ":" << it.second->statement;
        for (auto iter : it.second->parent)
            CD << "|" << iter->line_number << ":" << iter->statement;
        CD << "\n";
        T << it.second->line_number << ":" << it.second->statement;
        for (auto iter : it.second->transitive_edge)
            T << "|" << iter->line_number << ":" << iter->statement;
        T << "\n";
        I << it.second->line_number << ":" << it.second->statement;
        for (auto iter : it.second->interference_edge)
            I << "|" << iter->line_number << ":" << iter->statement;
        I << "\n";
        AR << it.second->line_number << ":" << it.second->statement;
        for (auto iter : it.second->affect_return_edge)
            AR << "|" << iter->line_number << ":" << iter->statement;
        AR << "\n";
        PI << it.second->line_number << ":" << it.second->statement;
        if (it.second->parametere_in_edge)
            PI << "|" << it.second->parametere_in_edge->line_number << ":" << it.second->parametere_in_edge->statement;
        PI << "\n";
        PO << it.second->line_number << ":" << it.second->statement;
        if (it.second->parametere_out_edge)
            PO << "|" << it.second->parametere_out_edge->line_number << ":" << it.second->parametere_out_edge->statement;
        PO << "\n";
        CE << it.second->line_number << ":" << it.second->statement;
        if (it.second->calling_edge)
            CE << "|" << it.second->calling_edge->line_number << ":" << it.second->calling_edge->statement;
        CE << "\n";
        RL << it.second->line_number << ":" << it.second->statement;
        if (it.second->return_link)
            RL << "|" << it.second->return_link->line_number << ":" << it.second->return_link->statement;
        RL << "\n";
    }
    slice.close();
    CD.close();
