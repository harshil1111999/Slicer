...
def TargetFileRunner(status,codepreview,nongraph,stdoutoutput,variablenames,output,InputEntry,flag):
...
def SliceCopy(output):
    ...
    tempwin.destroy()
def SourceGraph():
    espn=subprocess.run(["test.exe"],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
    GraphMaker("Source File System Dependence Graph")
    
def Main(targetfile):
    window=tk.Tk()
    window.geometry("%dx%d+0+0" % (window.winfo_screenwidth()-50,window.winfo_screenheight()-100))
    ...
    InputFrame.pack()
    runbutton=tk.Button(SelectAlgo,text="Run",command=lambda: TargetFileRunner(status,codepreview,nongraph,stdoutoutput,variablenames,output,InputEntry,flag))
    runbutton.pack()
